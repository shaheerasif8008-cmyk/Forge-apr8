...
def create_task_plan(
    *,
    ...
    for pack in packs:
        ...
        output_template = (
            output_template
            or pack.output_templates.get(lane)
            or pack.output_templates.get("hybrid", "")
        )
    ...
    expected_deliverables = ["brief_card"]
    if lane in {"business_process", "hybrid"}:
        expected_deliverables.append("action_log")
    else:
        expected_deliverables.append("written_deliverable")
...
def estimate_roi(
    ...
    *,
    ...
    minutes_per_task = max(
        [pack.roi_metrics.get("default_minutes_saved", 20.0) for pack in packs] or [20.0]
    )
    penalty = rework_events * 15.0
    return {
